...
Если нужно запустить API без падения на старте (например, для проверки роутов/шаблонов), по умолчанию fail-fast уже отключен, но при необходимости можно явно указать это:
...
Чтобы вернуть строгое поведение (падать при недоступной БД на старте):

```text
DB_FAIL_FAST_ON_STARTUP=true
```
